...
export class CodeComponent {
  @Input() width: any;
  @Input() height: any;
  @Output() sendCode: EventEmitter<any> = new EventEmitter<any>();
  result = '';
  ...
  randomColor = () => {
    return 'rgba(' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ',' + this.randomRange(70, 100) * .01 + ')';
  }
  getCode () {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.fillText(res, can.width / 15 + can.width / 4 * i, can.height / 2);
    }
